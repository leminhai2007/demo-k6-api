...
import { getUniqueData, combineData, getSequentialData } from './utils/data.js'
...
function dataParameterize(iteration, user) {
  const userData = getUniqueData(userParameters, user);
  const productData = getSequentialData(productParameters, iteration);
  
  return combineData(userData, productData);
}

// Actions for VU
export function scenarioS001() {
  const data = dataParameterize(scenario.iterationInTest, vu.idInTest - 1);
  testT001(data);
}
